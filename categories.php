...
$beautyMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'beauty'";
$beautyMarketsResult = $conn->query($beautyMarketsQuery);

// Fetch bakery markets
$bakeryMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'bakery'";
$bakeryMarketsResult = $conn->query($bakeryMarketsQuery);

// Fetch Home & garden markets
$homeandgardenMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'home & garden'";
$homeandgardenMarketsResult = $conn->query($homeandgardenMarketsQuery);

// Fetch electronics markets
$electronicsMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'electronics'";
$electronicsMarketsResult = $conn->query($electronicsMarketsQuery);

// Fetch grocery markets
$groceryMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'grocery'";
$groceryMarketsResult = $conn->query($groceryMarketsQuery);

// Fetch food and dining markets
$foodanddiningMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'food & dining'";
$foodanddiningMarketsResult = $conn->query($foodanddiningMarketsQuery);

// Fetch retail shopping markets
$retailandshoppingMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'retail & shopping'";
$retailandshoppingMarketsResult = $conn->query($retailandshoppingMarketsQuery);

// Fetch entertainment & arts markets
$entertainmentandartsMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'Entertainment & Arts'";
$entertainmentandartsMarketsResult = $conn->query($entertainmentandartsMarketsQuery);

// Fetch business to business markets
$businesstobusinessMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'business to business'";
$businesstobusinessMarketsResult = $conn->query($businesstobusinessMarketsQuery);

// Fetch education markets
$educationMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'education'";
$educationMarketsResult = $conn->query($educationMarketsQuery);

// Fetch recreation markets
$recreationMarketsQuery = "SELECT MarketID, Marketname, category, location, contacts, description, ratings, status, image, link, workinghours, FAQs FROM markets WHERE category = 'recreation'";
    ...
    <ul>
      ...
      if ($beautyMarketsResult->num_rows > 0) {
        ...
        while ($row = $beautyMarketsResult->fetch_assoc()) {
            ...
            echo '<p>' . $row["status"] . '</p>';
echo '<a href="includes/market.php">Visit Market</a>';
    ...
    <div class="categories-container">
        ...
        if ($bakeryMarketsResult->num_rows > 0) {
            ...
            while ($row = $bakeryMarketsResult->fetch_assoc()) {
                ...
                echo '<p>' . $row["ratings"] . '</p>';
                echo '<p>' . $row["status"] . '</p>';
                echo '<a href="markets.php">Visit Market</a>';
